...
const DualBlur: FC<IProps> = (props) => {
  const downRt = useRef<WebGLRenderTarget[]>([]);
  const upRt = useRef<WebGLRenderTarget[]>([]);

  const effect = useMemo(() => {
    downRt.current.forEach((rt) => rt.dispose());
    upRt.current.forEach((rt) => rt.dispose());
    return new DualBlurEffect(props, downRt.current, upRt.current);
  }, [props]);
